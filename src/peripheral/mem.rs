...
mod tests {
    ...
    fn test_load_store_word() {
        ...
        for _ in 0..100 {
            ...
            ram.store_word(address.clone(), &word);
            assert_eq!(&word, &ram.load_word(address.clone()));
        }
    }
    ...
    fn test_load_store_half() {
        ...
        for _ in 0..100 {
            ...
            let address: usize = rng.gen_range(BASE_ADDRESS..RAM_SIZE - 4);
            ram.store_half(address.clone(), &half);
            assert_eq!(&half, &ram.load_half(address.clone()));
        }
    }
    ...
    fn test_load_store_byte() {
        ...
        for _ in 0..100 {
            ...
            let address: usize = rng.gen_range(BASE_ADDRESS..RAM_SIZE - 4);
            ram.store_byte(address.clone(), &byte);
            assert_eq!(&byte, &ram.load_byte(address.clone()));
        }
    }
}
